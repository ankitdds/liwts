	...
	class BFI_Thumb_1_3 {
	    ...
	    public static function thumb( $url, $params = array(), $single = true ) {
	        ...
	        if ( @file_exists( $destfilename ) && getimagesize( $destfilename ) ) {
	            ...
	            /*****************CODE TO OPTIMISE IMAGES RUNTIME USING WP SMUSH PLUGIN API BY SAFALWEB - 20-06-2016********************************/
	            global $WpSmush;
	            $smushResult = $WpSmush->do_smushit($destfilename);
	            if(is_array($smushResult) && isset($smushResult['success']) && $smushResult['success'] == 1){
	            	if(isset($smushResult['data'])){
	            		$imgData = $smushResult['data'];
	            		if($imgData->image){
	            			file_put_contents($destfilename,$imgData->image);
	            		}
	            	}
	            }
	            /*****************CODE TO OPTIMISE IMAGES RUNTIME USING WP SMUSH PLUGIN API BY SAFALWEB - 20-06-2016********************************/
