...
require_once __DIR__ . '/MatthiasMullie/Minify/Minify.php';
require_once __DIR__ . '/MatthiasMullie/Minify/CSS.php';
require_once __DIR__ . '/MatthiasMullie/Minify/JS.php';
require_once __DIR__ . '/MatthiasMullie/Minify/Exception.php';
		...
		if ( ! file_exists( $combined_file_path ) ){
			if ( file_put_contents( $combined_file_path, $done_imploded ) ){
				if($extension == "css"){
					...
					$minifier->add($combined_file_path);
					$minifier->minify($combined_file_path);
				}
				if($extension == "js"){
...
function minit_resolve_css_urls( $content, $object, $script ) {
	...
	/* echo sprintf( "url('%s/$1')", $object->base_url . dirname( $src ) );
	print_r($src);
	exit; */
